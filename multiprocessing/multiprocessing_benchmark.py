...
def get_data_from_csv(data_path: Path, task_type: TaskTypesEnum, size: int = None, shuffle_flag: bool = True):
    data_frame = pd.read_csv(data_path)
    if size:
        data_frame = data_frame[:size]

    dataset_name = os.path.split(data_path)[-1]
    if dataset_name in ['jasmine.csv']:
    ...
    for n_p in processes:
        ...
        for _ in range(mean_range):
            ...
            try:
                ...
                auto_model.history.save(f'{path}/{n_p}_{_}.json')
...
def compare_n_process(train_data, test_data,  path='res', timeout=60):
    """
    Performs experiment to show how how multiprocessing increases the performance
    """
    pipelines_count, times, fitnesses, fitnesses_cv = [{timeout: []} for _ in range(4)]
    params = {
        'with_tuning': False, 'num_of_generations': 1000000, 'early_stopping_generations': 100000,
    ...
    _show_performance_plot(f'Dependency of number of pipelines from number of jobs', processes,
                           pipelines_count, times, path)
    _show_fitness_plot(f'Dependency of the best fitness from number of jobs', processes, fitnesses, fitnesses_cv,
                       times, path)
...
if __name__ == "__main__":
    for dataset in os.listdir('data'):
        ...
        compare_n_process(train_data, test_data,  path=f'res_multiproc/{dataset}', timeout=1)
